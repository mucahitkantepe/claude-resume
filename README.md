...
Claude Code lets you resume sessions, but finding the right one gets hard fast — especially once you have hundreds. Sessions older than 30 days are deleted by default, and there's no way to search across session content. `claude-resume` adds full-text search across every conversation you've ever had — messages, tool output, code snippets — with a fuzzy-matching TUI that lets you find and resume any session in seconds.
...
claude-resume search "auth" -n 5    # Limit results
claude-resume --force               # Force full index rebuild
claude-resume init                  # Configure Claude Code (cleanupPeriodDays, hooks)
